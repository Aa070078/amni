...
| Date | Change |
|---|---|
| 2026-08-16 | **M7-001 merged to dev** — PR #63 squash as `ab2af60` (agent-amni-01): seed-saas-admin.ts + platform-admin → `/admin` redirect + quick-login demo accounts. M7-002 (company seed) still in-progress by agent-m7-company-demo; branch not yet on remote. |
| 2026-08-16 | **M7-001 implemented** (agent-amni-01, PR #63 on `feat/M7/saas-admin-demo`): `seed-saas-admin.ts` (`owner@amni.com`, platform admin, no membership) + platform-admin post-login redirect → `/admin` in login-form + quick-login (demo accounts updated: SaaS Admin `owner@amni.com`, Company Admin `admin@demo.amni`, Company Member `member@demo.amni`). Live-verified login/auth-me/admin API 200. M7-002 (company seed) still in-progress by agent-m7-company-demo. |
