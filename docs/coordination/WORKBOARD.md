...
| **M10-009 v16 bench compat + HRMS install + full system test** | M10 | agent-amni-01 | done | fix/v16-bench-compat | Root-caused "couldn't load data" (bridge missing + hrms-less Expense Claim 404s); finance/expenses degrade to empty datasets on `erp_not_found`; removed v16-removed fields (`Purchase Invoice.purchase_order`, `Purchase Order.notes`, `Payment Entry.bill_no`); fixed flat-series NaN in chart; reseeded `owner@amni.com`. Installed hrms v16 on bench, aligned Expense Claim + Journal Entry field maps with real hrms schema, granted HR roles to bench users, replaced broken HRMS iframe with direct SSO button, fixed query cache stale-session bug on login. Full system test: 35 API endpoints, 21 web pages, auth flows, ERP bridge, HRMS SSO all verified. Added platform admin full-access: user list/detail/suspend/activate/promote/demote + tenant suspend/resume/archive/change-plan (10 new endpoints, 2 new frontend pages). [PR #81](https://github.com/Aa070078/amni/pull/81) |
| **M10-010 Release-gate CI SSO secret** | M10 | codex | in-progress | fix/ci-release-sso-secret | Supply an ephemeral valid AMNI_SSO_SECRET to the clean-site CI bootstrap; no secret is committed. |
